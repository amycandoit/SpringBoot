...
//						.and(ageLoe)
	...
	void test2() {
		...
		JPAQueryFactory queryFactory = new JPAQueryFactory(entityManager);
		String name = "na";
		BooleanExpression nameEq2 = name != null ? member.name.ne(name) : null;
		member.name.like("%" + name + "%");
		JPAQuery<Member> from = queryFactory
				.select(member)
				.from(member)
				.where(member.age.loe(10).and(member.age.gt(5)).and(nameEq2));
		List<Member> fetch = from.fetch();
		System.out.println();
	}
	...
	void test6() {
		...
		// select * from member innerJoin todos where name
		//select * from todo where todo.member.name = "name" and todo.likecount >=? and todo.contain like %t%
//		QMember qMember = QMember.member;
		...
		List<Todo> fetch = queryFactory.selectFrom(qTodo)
				.where(qTodo.member.name.eq("name")
						...
						, qTodo.content.contains("t")).fetch();
		Assertions.assertEquals(fetch.size(),30);
	}

	@Test
	void test7() {
		//select * from members m left join todos t on t.member_id = m.id
		//where m.name = "name" and t.like_count >= 10 and t.content like %t%
		QMember qMember = QMember.member;
		JPAQueryFactory queryFactory = new JPAQueryFactory(entityManager);
		QTodo qTodo = QTodo.todo;
